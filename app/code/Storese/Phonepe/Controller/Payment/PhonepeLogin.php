...
class PhonepeLogin extends \Magento\Framework\App\Action\Action
{
    ...
    public function initiateLogin($params)
    {
        $response = json_decode($this->callPhonePeForAuthToken($request));
        if ($response->success == true && $response->code == "SUCCESS") {
            $accessToken = $response->data->accessToken;
            $loginRequest = json_decode($this->getUserDetailsFromPhonePe($accessToken, $brandId));
